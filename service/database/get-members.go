...
var FindGroupMember = `SELECT UserId FROM user_group WHERE GroupId  = ?`

// Function used to get all members of a group with the user_group table
func (db appdbimpl) GetMembers(GroupId  string) ([]User, error) {
	// Get the members of a group
	rows, err := db.c.Query(FindGroupMember, GroupId )
	if err != nil {
		return nil, err
	}
