...
export default {
  ...
  methods: {
    ...
    async commentMessage(emoji) {
      ...
      const url = `/chats/${sessionStorage.userID}/${this.convId}/comment`;
      this.$axios.put(url, {messageId: this.msg.messageId,  comment: emoji }, { headers: { 'Authorization': sessionStorage.token } })
        .then(() => {
          // Chiude il modale
          this.closeModal();
        })
