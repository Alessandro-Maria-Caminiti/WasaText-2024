...
export default {
  ...
  if (this.groupName.length < 3 || this.groupName.length > 16) {
    this.errorMsg = "Invalid group name, it must contain min 3 characters and max 16 characters";
    return;
  }
  ...
  try {
    ...
    const response = await this.$axios.post(
      `/chats/${sessionStorage.userID}/creategroup`,
      {Name:  this.groupName},
      { headers: { 'Authorization': sessionStorage.token } }
    );

    // Supponendo che la tua API risponda con { groupId, conversationId, groupName }
    console.log(response.data)
    const {groupId, groupName,grouphoto } = response.data;
    
    localStorage.clear();
    localStorage.userID = groupId;
    localStorage.username = this.groupName;
    localStorage.grouphoto = grouphoto
    localStorage.convid = response.data.ConversationId
    this.closeModal();
    this.$router.push(`/chats/${sessionStorage.userID}/${localStorage.convid}`);
  } catch (e) {
    this.errorMsg = e.toString();
  }
},
